...
/*==*/
int
Barparser::ParseBarfile(FILE *fp, BarData **lmin)
{
...
int Barparser::MxReadBinRates(FILE *rate_file, double **rate_mx, int nstates, int max)
{
  int dimension = 0;
        int ref;
...
/*==*/
int
Barparser::ParseInfile(FILE *infile_fp, FILE *mr_FP, double **microrates)
{
  char *line=NULL;
...
/*==*/
int Barparser::ParseRatesFile(FILE *rates_FP, double **Raten)
{
  ...
  if (i!=my_dim-1 || j!=my_dim-1) {
    if (!_opt->quiet) fprintf(stderr, "WARNING: dimensions are corrupted lines: %d(%d); rows: %d(%d)\n", j, my_dim-1, i, my_dim-1);
  }
