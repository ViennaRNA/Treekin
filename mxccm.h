...
public:
   ...
   */
  template<typename T>
  void mcopy(T *a,T *b,int m);

  /**
   * Invert (in place) a general real matrix A -> Inv(A).
   */
  template<typename T>
  static int minv(T *a,int n, double epsilon);
   ...
   */
  template<typename T>
  static void mmul_singular(T *c,T *a,T *b,int dim1, int dim2, int dim3, int verbose);
...
template<typename T>
int
Mxccm::qrevec(T *ev,T *evec,T *dp,int n, double epsilon)
{
  ...
  for(j=0,m=n-1;; ++j) {
    while(1) {
      if(m<1) return 0;
      k=m-1;
      if(std::abs(dp[k])<=abs(ev[m])*tzr) --m;
      else {
        x=(ev[k]-ev[m])/2.;
        h=sqrt(x*x+dp[k]*dp[k]);
        if(m>1 && abs(dp[m-2])>abs(ev[k])*tzr) break;
...
template<typename T>
void
Mxccm::housev(T *a,T *d,T *dp,int n)
{
  T sc,x,y,h;
  int i,j,k,m,e;
  T *qw,*pc,*p;
...
template<typename T>
void
Mxccm::mmul_singular(T *c,T *a,T *b,int dim1, int dim2, int dim3, int verbose)
{
  T sum =0.0;
  int i,j,k;
  for (i = 0; i<dim1; i++) {
...
template<typename T>
void
Mxccm::vmul(T *vp,T *mat,T *v,int n)
{
  T s,*q;
  int k,i;
  for(k=0; k<n; ++k) {
...
template<typename T>
int
Mxccm::minv(T *a,int n, double epsilon)
{
  int lc, *le;
  ...
  for(j=0,pa=pd=a; j<n; ++j,++pa,pd+=n+1) {
    ...
    *le++ =lc;
    ...
    for(k=j+1,ps=pd,t=(T)(1./ *pd); k<n; ++k) *(ps+=n)*=t;
    *pd=t;
  }
...
template<typename T>
int
Mxccm::solv(T *a,T *b,int n, double epsilon)
{
  int i,j,k,lc;
  T *ps,*p,*q,*pa,*pd;
  T *q0;
