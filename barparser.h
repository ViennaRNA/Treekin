...
public:
  Barparser(treekin_options *opt, int lmins, SubInfo *E){
    _opt = opt;
    _lmins = lmins;
    _E = E;
  }
  ...
  template<typename T>
  void MxFPrintD(T *mx, const char *name, int dim1, int dim2, FILE *out);
  template<typename T>
  void MxOneShorten(T **shorten, int fulldim);
  template<typename T>
  void MxRShorten(T **shorten, int fulldim, int gdim);
  // matrix shortening routine (when nstates specified)
  template<typename T>
  int MxShorten(T **shorten, int nstates, int my_dim, int max);
  ...
  int ParseRatesFile(FILE *rates_FP, double **Raten);

  // visualisation
  template<typename T>
  void VisualizeRates(char *filename, T *R, BarData *Data, int dim);
...
template<typename T>
inline
void Barparser::MxFPrintD(T *mx, const char *name, int dim1, int dim2, FILE *out)
{
  int k, l;
  fprintf(out,"%s:{\n", name);
  for (k = 0; k < dim1; k++) {
...
template<typename T>
inline
void Barparser::MxRShorten(T **shorten, int fulldim, int gdim)
{
...
*/
  ...
  if (_opt->want_verbose) MxFPrintD(result2, "gb*bb-1*bg", gdim, gdim, stderr);
...
template<typename T>
inline
void Barparser::MxOneShorten(T **shorten, int fulldim)
{
  //does: shortened = GG - GB*BB^(-1)*BG, where matrix tmp_rates is split as:
  //tmp_rates = (GG | GB)
  //            (BG | BB)
...
template<typename T>
inline
int Barparser::MxShorten(T **shorten, int nstates, int my_dim, int max) {
  // shorten the matrix from dimension my_dim to nstates:
  if (my_dim>nstates) {
    if (!_opt->quiet) fprintf(stderr, "decreasing %d to %d\n", my_dim, nstates);
    ...
    if (max!=1) {
      while (my_dim-max > nstates) {
        ...
        if (!_opt->quiet) fprintf(stderr, "%d done...\n", my_dim-max);
        my_dim -= max;
      }
      ...
      while (my_dim!=nstates) {
        ...
        my_dim--;
        if (!_opt->quiet && my_dim%100==0 && my_dim>0) fprintf(stderr, "%d done...\n", my_dim);
      }
    }
  }
...
template<typename T>
inline
void Barparser::VisualizeRates(char *filename, T *R, BarData *Data, int dim)
{
  FILE *dotf;
