...
private:
  ...
  void    MxASCIIWrite(T          *Mx,
                       const char *asciifile);


  void    MxASCIIWriteV(T           *Mx,
                        const char  *asciifile);
...
public:
  ...
  T PrintProbFull(T       *line,
                  int     dim,
                  double  time,
                  int     lmins);


  T PrintProbNR(T       *line,
                int     dim,
                double  time);
  ...
  T *MxBar2Matrix(BarData *Data,
                  T *);


  void MxEqDistr(BarData  *Data,
                 T        **p8);


  void MxEqDistrFULL(SubInfo  *E,
                     T        *p8);


  void MxDiagonalize(T  *U,
                     T  **_S,
                     T  *PI);
  ...
  void MxEqDistrFromLinSys(T  *U,
                           T  **p8);


  void MxEqDistrFromDetailedBalance(T *U,
                                    T **p8);
...
template<typename T>
Calc<T>::Calc(Globals *globalParameters,
              SubInfo *E,
              int     d)
{
                ...
                int dim)
{
  ...
  /* check ergodicity */
  if ((int)set_ergo.size() == dim) {
    return dim;                              /* all ok */
  } else {
    int i = first + 1;
    while (i < dim) {
      if (p0[i] > (T)0.0 && set_ergo.count(i) == 0) {
        /* disconnected :( */
        fprintf(stderr,
                "ERROR: Matrix is non-ergodic and initial populations are disconected!! Exiting...\n");
        exit(-1);
      }
                       ...
                       int    lmins)
{
  ...
  for (int i = 0; i < lmins; i++) {
    if (ptFULL[i] < -0.01) {
      fprintf(stderr,
              "prob of lmin %i at time %e has become negative: %e \n",
              i + 1,
              time,
              (double)ptFULL[i]);
      if (_opt->num_err == 'H')
  ...
  if (((check - 1.) < -0.05) || ((check - 1.) > 0.05)) {
    fprintf(stderr, "overall probability at time %e is %e != 1.0 %s!\n", time, (double)check,
            (_opt->num_err == 'R' ? "rescaling" : "exiting"));
    if (_opt->num_err == 'H' || check == 0.0)
      exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < lmins; i++) {
    if (_opt->num_err == 'R')
      printf("%e ", (double)(T)(abs(ptFULL[i]) / check));
    else
      printf("%e ", (double)(T)abs(ptFULL[i]));
  }
  printf("\n");
                   ...
                   double time)
{
  ...
  if (((check - 1.) < -0.05) || ((check - 1.) > 0.05)) {
    fprintf(stderr, "overall probability at time %e is %e != 1.0 %s!\n", time, (double)check,
            (_opt->num_err == 'R' && (double)check != 0.0 ? "rescaling" : "exiting"));
    if (_opt->num_err == 'H' || check == 0.0)
      exit(EXIT_FAILURE);
  }
  ...
  if (reorganize.size() == 0) {
    for (int i = 0; i < dim; i++) {
      /* map individual structure -> gradient basins */
      if (_opt->num_err == 'R')
        printf("%e ", (double)(T)(abs(line[i]) / check));
      else
        printf("%e ", (double)(T)abs(line[i]));
    }
  } else {
    int j = 0;
    for (int i = 0; i < dim; i++) {
      if (j > (int)reorganize.size() || reorganize[j] != i) {
        ...
        if (_opt->num_err == 'R')
          printf("%e ", (double)(T)(abs(line[j]) / check));
        else
          printf("%e ", (double)(T)abs(line[j]));
...
template<typename T>
T *
Calc<T>::MxBar2Matrix(BarData *Data,
                      T       *R)
{
  T *U = NULL;

  if (_opt->want_degenerate)
    MxDoDegeneracyStuff();
                   ...
                   T        **p8)
{
  ...
  if (_opt->absrb) {
    ...
    for (i = 0; i < dim; i++)
      Z += exp(-((Data[i].energy - mfe) / _kT));
    for (i = 0; i < dim; i++)
      (*p8)[i] = exp(-((Data[i].energy - mfe) / _kT));
  }
...
template<typename T>
void
Calc<T>::MxEqDistrFULL(SubInfo  *E,
                       T        *p8)
{
  int i;
  T   Z = 0.;
  ...
  if (_opt->absrb) {
    ...
    for (i = 0; i < dim; i++)
      Z += exp(-E[i].energy / _kT);
    for (i = 0; i < dim; i++)
      p8[i] = (T)(exp(-E[i].energy / _kT) / Z);
  }
...
template<typename T>
void
Calc<T>::MxEqDistrFromLinSys(T  *U,
                             T  **p8)
{
  ...
  if (_opt->absrb) {
    ...
    *(*p8 + (dim - 1)) = 1.0; /* last entry is the 'new' absorbing state */
    ...
    if (nfo != 0) {
      fprintf(stderr,
              "dgesv exited with value %d (Cannot compute the equilibrium distribution) - check if the rates have good dimension(you have probably reached the numerical precision of treekin)\n",
              nfo);
    ...
    if (_opt->want_verbose)
      MxPrint(B, "X in MxEqDistrFromLinSys", 'v');
    ...
    *p8[0]  = 1.;
    ...
    for (i = dim - 1; i != 0; i--) {
      if ((*p8)[i] < 0.0) {
        ...
        fprintf(stderr,
                "Warning: p8[%5d] is negative (%e), setting it to a nearby value (%e)\n",
                i + 1,
                (*p8)[i],
                (i == dim - 1 ? (*p8)[i - 1] : (*p8)[i + 1]));
        (*p8)[i] = abs((i == dim - 1 ? (*p8)[i - 1] : (*p8)[i + 1]));
      }

      if ((*p8)[i] == 0.0) {
        fprintf(stderr,
                "Warning: p8[%5d] is zero (%e), setting it to a minimum value (%e)\n",
                i + 1,
                (*p8)[i],
                1e-20);
        (*p8)[i] = 1e-20;
                       ...
                       T  *P8)
{
  ...
  if (_opt->dumpMathematica == 1)
    MxKotzOutMathematica(U);
  ...
  if (!_opt->absrb) {
    ...
    if (_opt->want_verbose)
      MxPrint(_sqrPI, "_sqrPI", 'm');

    if (_opt->want_verbose)
      MxPrint(tmpMx, "tmpMx = _sqrPI*U", 'm');

    if (_opt->want_verbose)
      MxPrint(sqrPI_, "sqrPI_", 'm');

    /*memset(U,0,dim*dim*sizeof(T)); */
    fill_n(U, dim * dim, 0);
    mxccm.mmul(U, tmpMx, sqrPI_, dim);
    if (_opt->want_verbose)
      MxPrint(U, "U = _sqrPI*U*sqrPI_ (this should be symmetric, but still uncorrected)", 'm');
    ...
    if (std::isnan((double)err)
      ...
      fprintf(stderr,
              "Rates are not good -- check your rates (maybe the equilibrium was not computed correctly? rerun with --equil-file or -v set)!!!... (%Lf)\n",
              (long double)err);
      exit(-1);
    }
    ...
    if (!_opt->quiet)
      fprintf(stderr, "Corrected numerical error: %e (%e per number)\n", (double)err,
              (double)(T)(err / (T)(dim * dim)));

    if (_opt->want_verbose)
      MxPrint(U, "force symmetrized U", 'm');
  }
  ...
  if (reachable) {
    fprintf(stderr,
            "WARNING: Eigenvector matrix is non-ergodic (unreachable states from 1st state):");
    for (i = 0; i < dim; i++)
      if (!reachable[i])
  ...
  if (!_opt->quiet) {
    if ((T)abs(evals[0] - ((_opt->useplusI) ? 1. : 0.)) > (T)10 * _opt->FEPS) {
      fprintf(stderr,
              "WARNING largest eigenvalue is %g, see evals.txt and evecs.txt\n",
              (double)evals[0]);
  ...
  if (i > 1)
    fprintf(stderr,
            "WARNING: There are multiple evals=%d.0 (%4d) (multiple population traps -- maybe we compute crap)\n",
            _opt->useplusI ? 1 : 0,
            i);
  ...
  *_S = evecs;
  ...
  if (_opt->useplusI)
    for (i = 0; i < dim; i++)
      evals[i] -= 1.0;                                           /* compensate 4 translation of matrix U */
}
...
template<class T>
int
Calc<T>::ConvergenceReached(T   *p8,
                            T   *pt,
                            int dim,
                            int full)
{
                   ...
                   T  *S)
{
  ...
  if (!_opt->absrb) {
    ...
    if (_opt->want_verbose)
      MxPrint(evals, "evals in MxIterate", 'v');
  ...
  if (_opt->t0 == 0.0) {
    if (_opt->method == 'F')
      PrintProbFull(p0, dim, 0.0, _lmins);
    else
  ...
  for (tt = inc; tt < _opt->t8 + inc; inc *= _opt->tinc, tt += inc) {
    ...
    if (!_opt->absrb)
      mxccm.vmul(pt, CL, tmpVec2, dim);
    else
      mxccm.vmul(pt, S, tmpVec2, dim);
    ...
    if (_opt->method == 'F')
      check = PrintProbFull(pt, dim, time, _lmins);
    else
      check = PrintProb(pt, dim, time);

    /*PrintProbNR(p8FULL, lmins, -1); */
    int reached;
    if (_opt->method == 'F')
      reached = ConvergenceReached(p8FULL, ptFULL, _lmins, 1);
    else
      reached = ConvergenceReached(p8, pt, dim, 0);
  ...
  if (_opt->warnings) {
    for (i = 0; i < dim; i++)
      if (underflow[i] > 0.0)
        fprintf(stderr, "underflow %5d at time %12g", i + 1, (double)underflow[i]);
  }

  if (time < _opt->t8) {
    if (_opt->method == 'F')
      PrintProbFull(pt, dim, _opt->t8, _lmins);
    else
...
template<typename T>
T *
Calc<T>::MxMethodeINPUT(BarData *Data,
                        T       *Input)
{
  int i, j;
...
template<typename T>
T
Calc<T>::max_saddle(int     i,
                    int     j,
                    BarData *Data)
{
  int tmp;

  if (Data[i].number > Data[j].father) {
    /* exchange i & j */
    tmp = i;
    i   = j;
    j   = tmp;
  ...
  if (Data[i].number == Data[j].father) {
    return (T)(Data[j].energy + Data[j].ediff);
  } else {
    if ((Data[j].energy + Data[j].ediff) >
        max_saddle((Data[j].father - 1), (Data[i].number - 1), Data))
      return Data[j].energy + Data[j].ediff;
    else
      return max_saddle((Data[j].father - 1), (Data[i].number - 1), Data);
  }
}
...
template<typename T>
void
Calc<T>::MxFixevecsAbsorb(T *evecs,
                          T *evals)
                       ...
                       const char *asciifile)
{
  ...
  if (!_opt->quiet)
    fprintf(stderr, "vector written to ASCII file\n");
                    ...
                    T *U)
{
  ...
  for (tt = _opt->t0; tt < _opt->t8 * _opt->tinc; tt *= _opt->tinc) {
    ...
    if (((check - 1.) < -0.01) || ((check - 1.) > 0.01)) {
      fprintf(stderr,
              "overall probability at time %e is %e != 1.0 %s!\n",
              (double)time,
              (double)check,
              (_opt->num_err == 'R' ? "rescaling" : "exiting"));
      if (_opt->num_err == 'H') {
               ...
               FILE *out)
{
  ...
  if (_opt->absrb) {
    ...
    MxFPrint(M,
             "First passage times (i-th row, j-th column represents fpt from i-th to j-th state)",
             'm',
             out,
             out != stderr);
    delete[] M;
...
Calc<T>::MxEVLapackSym(T *U)
{
  ...
  if (_opt->want_verbose)
    fprintf(stderr, "FEPS precision %20.10g\n", (double)abstol);
  ...
  if (nfo != 0) {
    fprintf(stderr,
            "dsyevx exited with value %d (val=%20.10g) (Cannot compute eigenvalues) Try to:\n - lower --feps value (try --feps=-1.0)\n - try using --useplusI option (sometimes the eigenvalues are better computed like that)\n",
            nfo,
            (double)evals[nfo - 1]);
    for (il = 0; il < dim; il++)
      fprintf(stderr, "%20.10g ", (double)evecs[nfo * dim + il]);
    fprintf(stderr, "\n");
...
inline
void
Calc<T>::MxEqDistrFromDetailedBalance(T *U,
                                      T **p8)
{
  T   *res = *p8;
  int *done;
...
Calc<T>::print_settings(void)
{
  ...
  if (_opt->basename != NULL)
    ...
    printf("# Basename: <stdin>\n");

  if (_opt->tinc)
    printf("# Time increment: %.2f\n", _opt->tinc);
  else
    printf("# Time increment: %.2f \n", _opt->tinc);

  if (_opt->want_degenerate == 1)
    printf("# Degeneracy:  on\n");
  else
    printf("# Degeneracy:  off\n");

  if (_opt->absrb < 1)
    printf("# Absorbing state: none\n");
  else
    printf("# Absorbing state: %d\n", _opt->absrb);

  if (_opt->n > 0)
    printf("# States limit: %d\n", _opt->n);
  else
    printf("# States limit: till EOF\n");
}
      ...
      while (current < nr) {
        for (b = current + 1; b <= nr; b++) {
          ...
          if (saddle[count].cc > 1) {
            D[dim * (saddle[count].list[current] - 1) +
              (saddle[count].list[b] - 1)] += (saddle[count].cc - 1);
